...
export function Room({ room }: RoomCardProps) {
    ...
    const { handleOpenModalRoomDetails, setSelectedRoom, selectedRoom, handleOpenModalRegisterGuest } = UhStore()
    const roomStatus = getRoomStatus(room);

    const handleOpenRoomDetails = async (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
        e.stopPropagation();
        if (!room.reserve /* || room?.isRestricted */) {
    ...
    return (
        <div data-room={true} className={`w-36 h-[12rem] bg-white rounded-lg shadow-md  transition-transform duration-200  overflow-hidden cursor-pointer`} onClick={handleOpenRoomDetails} >
